...
import { Balance, Period, Position, Trade, Transaction } from '../data.dto'
...
export class KucoinService implements ExchangeService {
  ...
  loadSymbolKLines(symbol: string, period: Period): Promise<void> {
    throw new Error('Method not implemented.')
  }
  removeSymbolKLines(symbol: string, period: Period): Promise<void> {
    throw new Error('Method not implemented.')
  }
  ...
  startAutoRefresh(intervalMs = 60000): void {
    ...
    this.logger.debug(
      `KucoinService auto-refresh started: every ${intervalMs / 1000} seconds`,
    )
  }

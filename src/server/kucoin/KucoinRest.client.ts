...
export class KucoinRestClient {
  ...
  async fetchTransactions(): Promise<any[]> { //en Kucoin las transactions es el historial de posiciones
    this.logger.debug('Obteniendo Transactions de KuCoin Futures...');
    try {
      const req = Futures.Positions.GetPositionListReq.builder().build();
      const response = await this.positionsService.getPositionsApi().getPositionList(req);
  ...
  async fetchPositions(): Promise<any[]> {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        const cleanPositions: KucoinSdkPosition[] = response.data.map((pos: any) => ({
          id: pos.id,
          symbol: pos.symbol,
        ...
        this.logger.debug(`Posiciones obtenidas: ${cleanPositions.length} posiciones`);

...
const printData = (start: boolean) => {
  if (chart.value) {
    ...
    const decimals = priceSample > 1000 ? 0 : priceSample > 10 ? 2 : priceSample > 0 ? 4 : 6
    ...
    if (start) {
      ...
      const lastChartTimestamp =
        currentData.length > 0 ? currentData[currentData.length - 1].timestamp : 0
...
const filteredTrades = computed(() => {
  const minDate = Math.min(props.klines[0]?.time, props.klines[props.klines.length - 1]?.time)
  return props.trades.filter(
    x => new Date(x.filledTime).getTime() > subHours(new Date(minDate), 1).getTime()
  )
})
...
const draw = () => {
  ...
  if (!props.hideTrades) {
    filteredTrades.value.forEach(trade => {
      ...
      const overlayName = trade.side === 'BUY' ? 'simpleAnnotationDown' : 'simpleAnnotation'
  ...
  props.positions.forEach(position => {
    ...
    // Exit price at 1%
    const percentage = 1
    const exitPrice = positionPrice + ((isLong ? 1 : -1) * positionPrice * percentage) / 100
    chart.value.createOverlay({
      name: 'priceLine',
      groupId: 'positions',
...
<template>
  <div class="relative" v-element-size="updateChartSize">
    ...
    <div class="absolute uppercase bottom-10 text-slate-700 text-xl font-bold">
      {{ symbol }} - {{ periods }}
    </div>
